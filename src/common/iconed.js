...
const Iconed = ({ icon, theme, text, className, iconClassName, textClassName, right = false }) => {
    ...
    const iconElement = <Icon type={icon} theme={theme} className={`iconed-icon${withSpace(iconClassName)}`} />
    const textElement = <span className={`iconed-text${withSpace(textClassName)}`}>{text}</span>
    return (
